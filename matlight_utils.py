...
def discrete_autoregreesive_act(decoder, obs_rep, obs, batch_size, n_agent, action_dim, tpdv, available_actions=None, deterministic=False):
    ...
    output_action = torch.zeros((batch_size, n_agent, 1), dtype=torch.long) #先创建一个tensor
    output_action_log = torch.zeros_like(output_action, dtype=torch.float32)
    for i in range(n_agent):    # 取16次
        # print("--2022.10.7-->shifted action: ", shifted_action)
        logit = decoder(shifted_action, obs_rep, obs)[:, i, :]
        if available_actions is not None: # 暂时没有用到这里
            logit[available_actions[:, i, :] == 0] = -1e10
        # print("logit: ", logit) # 2022.8.27 好像是每一次step，每个智能体他们互相动作概率不一样，但是他们与上一个step的自己相比，4个action的概率一样
        distri = Categorical(logits=logit)
        # print("deterministic: ", deterministic) # 2022.8.27 False 随机性策略或确定性策略
        action = distri.probs.argmax(dim=-1) if deterministic else distri.sample()  #取argmax
...
def tile_images(img_nhwc):
    """
        将 N 个图像平铺成一个大的 PxQ 图像 (P,Q) 选择尽可能接近，如果 N 是正方形，则 P=Q。
    """
...
class SharedReplayBuffer(object):
    ...
    def __init__(self, num_agents, act_space, buffer_size, gamma, gae_lambda, n_rollout_threads, hidden_size, recurrent_N): # 2022.9.30 , obs_space, cent_obs_space, act_space
        self.episode_length = buffer_size
        ...
        self.available_actions = None # 2022.9.20 改为None，表示为所有动作可选
    ...
    def after_update(self):
        if self.episode_length < 360:
            self.share_obs[0] = self.share_obs[-1].copy()
            self.obs[0] = self.obs[-1].copy()
        else:
            self.share_obs[0] = np.zeros((self.episode_length + 1, self.n_rollout_threads, self.num_agents, *[12]), dtype=np.float32)[0]
            self.obs[0] = np.zeros((self.episode_length + 1, self.n_rollout_threads, self.num_agents, *[12]), dtype=np.float32)[0]
    ...
    def compute_returns(self, next_value, value_normalizer=None):
        if self._use_proper_time_limits:
            ...
            if self._use_gae:
                self.value_preds[-1] = next_value
                gae = 0
                for step in reversed(range(self.rewards.shape[0])): # 10 # 2022.9.24 逆序 [9, 8, 7, 6, 5, 4, 3, 2, 1, 0]
                    if self._use_popart or self._use_valuenorm:
                        if self.algo == "mat" or self.algo == "mat_dec":
                            ...
                            self.returns[step] = gae + value_t # 2022.9.26 V tar
                        else:
                            delta = self.rewards[step] + self.gamma * value_normalizer.denormalize(
                                self.value_preds[step + 1]) * self.masks[step + 1] \
                                    - value_normalizer.denormalize(self.value_preds[step])
    ...
    def feed_forward_generator_transformer(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        rand = torch.randperm(batch_size).numpy() # 将0~n-1（包括0和n-1）随机打乱后获得的数字序列
