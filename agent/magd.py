...
class MAGDAgent(RLAgent):
    ...
    def train(self):
        ...
        target_q_next = self.target_q_model(q_input_target, train=False)    #(256,300)

        target_q += rewards_list[self.rank] + self.gamma * target_q_next        # 每次进来 target_q = 0 += 没有意义
