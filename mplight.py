...
class MPLightAgent(RLAgent):
    ...
    def __init__(self, world, rank):
        ...
        self.phase_buffer = []  # 清零
        self.obs_buffer = []
        self.name = 'mplight'

        self.world = world
        self.rank = rank
        self.sub_agents = len(self.world.intersections)  # 分散
    ...
    def get_action(self, ob, phase, test=False):
        ...
        if self.valid_acts is None:
            ...
            batch_valid = [
                self.valid_acts.get(dic[i]) if dic[i] in self.valid_acts.keys() else self.valid_acts.get('GS_' + dic[i])
                for i in range(ob.shape[0])]
            batch_reverse = [
                self.reverse_valid.get(dic[i]) if dic[i] in self.reverse_valid.keys() else self.reverse_valid.get(
                    'GS_' + dic[i]) for i in range(ob.shape[0])]
    ...
    def remember_time(self, episode, phase, obs, time):
        phase_dimt = {"phase": phase.tolist(),}
        for i in range(len(obs)):
            obs[i] = obs[i].tolist()
        obs_dimt = {"obs": obs,}
        self.phase_buffer.append(phase_dimt)
        self.obs_buffer.append(obs_dimt)
        if len(self.phase_buffer) == 360:
            with open('./analysis_data/%s%s%d.json' % (self.name, '_phase_', episode,), 'w', encoding='utf8') as f1:
                json.dump(self.phase_buffer, f1)
                self.phase_buffer = []
            with open('./analysis_data/%s%s%d.json' % (self.name, '_obs_', episode,), 'w', encoding='utf8') as f2:
    ...
    def _build_model(self):
        ...
        agents = MPLight_InerAgent(self.model, self.optimizer, self.replay_buffer, self.gamma, self.explorer,
                                   ...
                                   target_update_interval=self.dic_agent_conf.param["target_update"] * self.sub_agents,
                                   update_interval=self.sub_agents
                                   )
        return agents
...
class MPLight_InerAgent(DQN):
    ...
    def batch_act(self, batch_obs, valid_acts=None, reverse_valid=None, test=False):
        ...
        if valid_acts is None:  # valid_acts : None
            return super(MPLight_InerAgent, self).batch_act(batch_obs)
        with torch.no_grad(), evaluating(self.model):
            batch_av = self._evaluate_model_and_update_recurrent_states(batch_obs)
            # TODO do not use gpu
            batch_qvals = batch_av.params[
                0].detach().cpu().numpy()  # shape: [sub_agents, 1+ob_length(remove right_lane?)]
            batch_argmax = []
