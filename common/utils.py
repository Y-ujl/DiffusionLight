...
def load_config(path, previous_includes=[]):
    ...
    # Load configs from included files. 从包含的文件加载配置。
    ...
    for include in includes:
        ...
        # Duplicates between includes causes an error includes之间的重复会导致错误
        config, merge_dup_error = merge_dicts(config, include_config)
        duplicates_error += merge_dup_error

    # Duplicates between included and main file causes warnings 包含文件和主文件之间的重复会导致警告
...
def build_config(args):
    ...
    # 从 configs/agents/{agent_name} 加载特定智能体的 configs 文件
    ...
    for key in args_dict:
        # update 将指定的项目插入到字典中
        config.update({key: args_dict[key]})  # short access for important param 重要参数的快捷访问

    # add network(for FRAP and MPLight)
    # 添加网络（用于 FRAP 和 MPLight）
